...
% shmuelsemtex's \catName/\catSeqName (auto-detecting single-token/
...
% shmuelsemtex's \morphCompose, \morphComposeHead, \morphComposeTail
...
% removed 2026-09-01 (second pass): shmuelsemtex's \morphMono was
...
% \morphEpi removed 2026-09-01, superseded by shmuelsemtex's \morphEpi
...
% --------------------------------------------------------------------
% equant/uquant/setupquant/set/setupset/union/intersection/full/
% maxfull/maximal/strict/Triv/seqname migrated into the
% shmuelsemtex package 2026-09-15 (cleaned up: separater typo
% fixed, scratch variables properly scoped per macro family --
% see shmuelsemtex.dtx Sec. "Quantifiers, sets, and
% underset-variant renderers"). Public interface unchanged, so
% no call site below needed to change. Id/ID/donotbreak/nobreaks/
% pagecrefn stay here -- not semantic markup, deliberately not
% migrated.
...
links are continuous functions $\morphName{f}_i \maps U_i \to U_{i+1}$
and $\morphName{f}'_j \maps U_j \to U_{j+1}$ between the sets:
...
D \defeq \{
  &
    U_0 = V_0 \to^{\morphName{f}_0} U_1,
    \dotsc,
    U_{m-1} \to^{\morphName{f}_{m-1}} U_m,
\\
  &
    U_0 = V_0 \to^{\morphName{f}'_0} V_1,
    \dotsc,
    V_{n-1} \to^{\morphName{f}'_{n-1}} V_n
\}
\end{split}
\end{equation}
...
\node um(-500,-1500)[U_m]
\node vn(500,-1500)[V_n]
\arrow |l|[u0`u1;\morphName{f}_0]
\arrow |r|[u0`v1;\morphName{f}'_0]
\arrow |l|[u1`d1;\morphName{f}_1]
\arrow |r|[v1`d2;\morphName{f}'_1]
\arrow |l|[d1`um;\morphName{f}_{m-1}]
\arrow |r|[d2`vn;\morphName{f}'_{n-1}]
\efig \]
...
$\hat{\morphName{f}} \maps U_m \to V_n \arin \catName{C}$
($\hat{\morphName{f}'} \maps V_n \to U_m \arin \catName{C}$)
...
\arrow |l|[d1`um;\morphName{f}_{m-1}]
\arrow |r|[d2`vn;\morphName{f}'_{n-1}]
\arrow |a|/{@{->}@/^5pt/}/[um`vn;\hat{\morphName{f}}]
\arrow |b|/{@{->}@/^5pt/}/[vn`um;\hat{\morphName{f}'}]
\efig \]
...
\end{equation}
\begin{equation}
\morphName{f}_i \restrictto_{U'_i} \maps U'_i \to U'_{i+1}
\end{equation}
\begin{equation}
\morphName{f}'_j \restrictto_{V'_j} \maps V'_j \to V'_{j+1}
\end{equation}
...
D' \defeq \{
  &
    U'_0 = V'_0 \to^{\morphName{f}_0} U'_1,
    \dotsc,
    U'_{m-1} \to^{\morphName{f}_{m-1}} U'_m,
\\
  &
    U'_0 = V'_0 \to^{\morphName{f}'_0} V'_1,
    \dotsc,
    V'_{n-1} \to^{\morphName{f}'_{n-1}} V'_n
\}
\end{split}
\end{equation}
\end{definition}
...
\arrow |r|/_{ (}->/[x`up0;i]
\arrow |a|/_{ (}->/[up0`u0;i]
\arrow |b|[up0`up1;\morphName{f}_0]
\arrow |b|[up0`vp1;\morphName{f}'_0]
\arrow |a|/^{ (}->/[up0`v0;i]
\arrow |l|[u0`u1;\morphName{f}_0]
\arrow |r|[v0`v1;\morphName{f}'_0]
\arrow |l|[u1`d1;\morphName{f}_1]
\arrow |b|/_{ (}->/[up1`u1;i]
\arrow |r|[up1`d2;\morphName{f}_1]
\arrow |b|/^{ (}->/[vp1`v1;i]
\arrow |l|[vp1`d3;\morphName{f}'_1]
\arrow |r|[v1`d4;\morphName{f}'_1]
\arrow |l|[d1`um;\morphName{f}_{m-1}]
\arrow |b|/_{ (}->/[d2`d1;i]
\arrow |r|[d2`upm;\morphName{f}_{m-1}]
\arrow |l|/^{ (}->/[d3`d4;i]
\arrow |l|[d3`vpn;\morphName{f}'_{n-1}]
\arrow |r|[d4`vn;\morphName{f}'_{n-1}]
\arrow |b|/_{ (}->/[upm`um;i]
\arrow |a|/{@{->}@/^5pt/}/[upm`vpn;\hat{\morphName{f}}]
\arrow |b|/{@{->}@/^5pt/}/[vpn`upm;\hat{\morphName{f}'}]
\arrow |b|/^{ (}->/[vpn`vn;i]
\efig \]
% \[\begin{tikzcd}
%   &
...
%   &
%   U'_0
%     \ar[ll, hook', swap, "\morphName{i}"]
%     \ar[dl, "\morphName{f}_0"]]
%     \ar[dr, swap, "\morphName{f}'_0"]
%     \ar[rr, hook,"\morphName{i}"]
%   &
%   &
%   U_0
%     \ar[d, "\morphName{f}'_0"]
...
%   &
%   U'_1
%     \ar[l, hook', "\morphName{i}"]
%     \ar[d, "\morphName{f}_1"]
%   &
%   &
%   V'_1
%     \ar[r, hook, "\morphName{i}"']
%     \ar[d, "\morphName{f}'_1"]
%   &
%   V_1
%     \ar[d, "\morphName{f}'_1"]
...
%   \vdots
%     \ar[r, hook, "\morphName{i}"']
%     \ar[d, "\morphName{f}'_{n-1}"]
%   &
%   \vdots \ar[d, "\morphName{f}'_{n-1}"]
% \\
%   U_m
%   &
%   U'_m
%     \ar[l, hook', "\morphName{i}"]
%     \ar[rr, dotted, "\iso", "\hat{\morphName{f}}"'] &
%   &
%   V'_n
%     \ar[r, hook, "\morphName{i}"']
%   &
...
If $\hat{\morphName{f}} \maps U_m \toiso V_n \arin \catName{C}$ then
$\hat{\morphName{f}} \arin \catName{C}'$
\end{proof}
\end{lemma}
...
$\morphName{f}_i \maps U_i \to U_{i+1}$ and
$\morphName{f}'_j \maps V_j \to V_{j+1}$ between the spaces:
...
If $\morphName{f} \maps \seqname{S}^1$ to $\seqname{S}^2$ is an
m-morphism of $\seqname{S}^1$ to $\seqname{S}^2$ then
$\morphName{f} \maps \seqname{S}^1$ to $\seqname{S}^2$ is a local
m-morphism of $\seqname{S}^1$ to $\seqname{S}^2$.

\begin{proof}
Since $\morphName{f} \maps \seqname{S}^1$ to $\seqname{S}^2$ is a
...
every function $\morphName{f} \maps U^1 \to U^2$ that is a local
...
Since $\catName{M}^2 \subcat[full-] \catName{M}^3$, $\morphName{f}^1$ is a
...
then $(U', V'', \phi' \morphCompose \phi \restrictto_{U'}) \in \seqname{A}$.
...
\pagecref{lem:M-CompatATL} is m-compatible with the charts of
$\seqname{A}$.  Since $\phi'$ is a model homeorphism,
$(U', V'', \phi' \morphCompose \phi)$ is m-compatible with the charts of
$\seqname{A}$.  Since $\seqname{A}$ is maximal,
$(U', V'', \phi' \morphCompose \phi)$ is a chart of $\seqname{A}$.
\end{proof}
...
$\morphSeqName{f}$ is an $\seqname{E}^1$-$\seqname{E}^2$ m-atlas near
...
$\hat{\morphName{f}} \maps V'^2 \toiso \hat{V}'^2$%
\footnote
{
  This reverses the direction of the arrow
  $\hat{\morphName{f}} \maps U_m \toiso V_n$ from
  \pagecref{def:NCD}.
  This is permissible since $\hat{\morphName{f}}$ is an isomorphism
}
...
\\
  &&
  \morphName{f}_0 \maps I \defeq U^1 \cap \morphName{f}^{-1}_0[U^2] \to U^2,
  \phi^2 \maps U^2 \toiso V^2,
\\
  &&
  \phi^1 \maps I \morphMono V^1,
  \morphName{f}_1 \maps  V^1 \to \seqname{C}^2
\\
& \Biggr \}
\end{array}
\end{equation}
...
\begin{equation}
\label{eq:fup1in}
\morphName{f}_0[U'^1] \subseteq U'^2
\end{equation}
...
\begin{equation}
\morphName{f}_1[V'^1] \subseteq \hat{V}'^2
\label{eq:fvp1in}
\end{equation}
...
\begin{equation}
\hat{\morphName{f}} \morphCompose \phi^2 \morphCompose \morphName{f}_0 =
\morphName{f}_1 \morphCompose \phi^1
...
\begin{figure}[ht]
\[ \bfig
\node uint(0,0)[{I = U^1 \cap \morphName{f}^{-1}_0[U^2]}]
\node u2(2000,0)[U^2]
\node v1(0,-1000)[V^1]
...
\[ \bfig
\node x(500,0)[u^1]
\node uint(-300,-500)[{I=U^1 \cap \morphName{f}^{-1}_0[U^2]}]
\node up1(500,-500)[U'^1]
\node up2(1500,-500)[U'^2]
\node u2(2000,-500)[U^2]
\node v1(-300,-1000)[V^1]
...
\arrow |a|/_{ (}->/[vp1`v1;\morphName{i}]
\arrow |a|[vp1`vhp2;\morphName{f}_1]
\arrow |a|/ >.>>/[vp2`vhp2;\hat{\morphName{f}}]
\arrow |b|//[vp2`vhp2;\iso]
\arrow |a|/^{ (}->/[vp2`v2;\morphName{i}]
%\arrow |a|/^{ (}->/[v2`c2;\morphName{i}]
\efig \]
...
$\morphSeqName{f}$ is an $E^1$-$E^2$ m-atlas near morphism of
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces
$\seqname{C}^1$, $\seqname{C}^2$, abbreviated as
$
  \isAtl[near]_\Ar
...
If $\morphSeqName{f}$ is an m-atlas morphism then $\morphSeqName{f}$ is an
...
If $\seqname{A}^i$, $i=1,2$, is semi-maximal and $\morphSeqName{f}$ is a
(semi-strict, strict)
\nobreaks
  {
    {($\catName{E}^1$-$\catName{E}^2$-)},
...
\leavevmode
\begin{description}
\item [$\morphSeqName{f}$ is an M-atlas morphism.]
...
$e^1 \in I \defeq U^1 \cap \morphName{f}^{-1}_0[U^2]$ there are model
neighborhoods $U'^1 \subseteq I$, $V'^1 \subseteq V^1$,
$U'^2 \subseteq U^2$, $V'^2 \subseteq V^2$, $\hat{V}'^2 \subseteq C^2$
and an isomorphism $\hat{\morphName{f}} \maps V'^2 \toiso \hat{V}'^2$
...
$
  (U'^2, \hat{\morphName{f}}[V'^2], \hat{\morphName{f}} \morphCompose \phi^2)
  \in \seqname{A}^2
$.

Define
$
  \phi'^2 \maps U'^2 \toiso \hat{\morphName{f}}[V'^2] \defeq
  \hat{\morphName{f}} \morphCompose \phi^2
$.
Then Diagram \eqref{eq:AtlM1} is commutative.

\item [$\morphSeqName{f}$ is (semi-strict, strict).]
...
$\hat{\morphName{f}} \maps ^{f}V'^2 \toiso \hat{^{f}V'^2}$ an isomorphism
...
$\hat{\morphName{g}} \maps ^{g}V'^2 \toiso \hat{^{g}V'^2}$ an isomorphism
...
\arrow |a|[fVp2`fVhp2;\hat{\morphName{f}}]
\arrow |b|[fVp2`fVhp2;\iso]
\arrow |a|/^{ (}->/[fVp2`V2f;\morphName{i}]
%
% y=-500
\arrow |r|[U1f`V1f;\phi^1]
\arrow |l|//[U1f`V1f;\iso]
...
\arrow |a|[gVp2`gVhp2;\hat{\morphName{g}}]
\arrow |b|[gVp2`gVhp2;\iso]
\arrow |a|/^{ (}->/[gVp2`V2g;\morphName{i}]
%
% y=-3000
...
\arrow |r|/_{ (}->/[Vp2`fVp2;\morphName{i}]
\arrow |r|/^{ (}->/[Vp2`gVp2;\morphName{i}]
%
% y=-2000
\arrow |a|[gVp1`gVhp2;\morphName{g}_1]
\arrow |a|[gVp2`gVhp2;\hat{\morphName{g}}]
...
\\
  &&
  \phi^1 \maps I \morphMono V^1,
  \morphName{f}^2_1 \morphCompose \morphName{f}^1_1 \maps  V^1 \to \seqname{C}^3,
\\
  &&
  \morphName{f}^2_0 \morphCompose \morphName{f}^1_0 \maps  I \to U^3,
...
$u^3 \defeq \morphName{f}^2_0(u^2)$.
$(U^2, V^2, \phi^2) \in \seqname{A}^2$ be a chart at $u^2$. Since
$\morphSeqName{f}^1$ is a morphism, there exists a subchart
$
  (U'^1, V'^1, \phi^1 \maps U'^1 \toiso V'^1)
  \in \seqname{A}^1
$
...
$
  ...
  \in \seqname{A}^2
$
at $u^2$ such that $\morphName{f}^1_0[U'^1] \subseteq U'^2$ and diagram
\eqref{eq:AtlM1} in \fullcref{def:M-ATLmorph} on \cpageref{eq:AtlM1} is
commutative, as shown in \pagecref{fig:AtlM1}.

Let $I^{2,3} \defeq U'^2 \cap \morphName{f}^{2-1}_0[U^3]$. Since
$\morphSeqName{f}^2$ is a near morphism, diagram \eqref{eq:morphcompN2} below
...
$\hat{\morphName{f}} \maps V'^3 \toiso \hat{V}'^3$\! such that diagram
...
\\
  &&
  \morphName{i} \maps \set{u^2} \morphMono I^{2,3},
  \phi'^2 \maps I^{2,3} \morphMono V'^2,
  \morphName{f}^2_1 \maps  V'^2 \to \seqname{C}^3,
\\
  &&
  \morphName{f}^2_0 \maps I^{2,3} \to U'^3,
  \phi^3 \maps U'^3 \toiso V'^3
\\
& \bigr \}
\end{array}
...
\end{equation}
\begin{equation}
\morphName{f}_0[U''^2] \subseteq U'^3
\end{equation}
\begin{equation}
\phi^1[U''^2] = V''^2
\end{equation}
\begin{equation}
\morphName{f}_1[V''^2] \subseteq \hat{V}'^3
\label{eq:fvp1in2}
\end{equation}
\begin{equation}
\phi^3[U'^3] = V'^3
\end{equation}
\begin{equation}
\hat{\morphName{f}} \morphCompose \phi^3 \morphCompose \morphName{f}^2_0 =
\morphName{f}^2_1 \morphCompose \phi'^2
\label{eq:fphigeqgphi2}
\end{equation}

Define $U''^1 \defeq \morphName{f}^1_0[U''^2]$ and
$V''^1 \defeq \morphName{f}^1_1[V''^2]$. Diagram \eqref{eq:morphcompN3}
...
\\
  ...
  \phi^1 \maps U''^1 \morphMono V''^1,
  \morphName{f}^2_1 \morphCompose \morphName{f}^1_1 \maps  V''^1 \to V'^3,
\\
  &&
  \morphName{f}^2_0 \morphCompose \morphName{f}^1_0 \maps U''^1 \to U'^3
  \phi^3 \maps U'^3 \toiso V'^3
\\
& \bigr \}
\end{array}
...
\end{equation}
\begin{equation}
\morphName{f}^2_0 \morphCompose \morphName{f}^1_0[U''^1] \subseteq U'^3
\end{equation}
\begin{equation}
\phi^1[U'^3] = V'^3
\end{equation}
\begin{equation}
\morphName{f}^2_1 \morphCompose \morphName{f}^1_1[V''^1] \subseteq \hat{V}'^3
\label{eq:fvp1in3}
\end{equation}
\begin{equation}
\phi^3[U'^3] = V'^3
...
\begin{array}{lll}
  \hat{\morphName{f}} \morphCompose \phi^3 \morphCompose \morphName{f}^2_0
      \morphCompose \morphName{f}^1_0
  &
    =
    &
      \morphName{f}^2_1 \morphCompose \phi'^2 \morphCompose \morphName{f}^1_0
\\
  &
    =
    &
      \morphName{f}^2_1 \morphCompose \morphName{f}^1_1 \morphCompose \phi^1
\end{array}
...
\node uint(0,-1000)[{I^{2,3} = U'^2 \cap \morphName{f}^{-1}_0[U^3]}]
...
\arrow |b|//[up1`vp1;\iso]
\arrow |r|[up1`up2;\morphName{f}^1_0]
\arrow |r|[vp1`vhp2;\morphName{f}^1_1]
\arrow |r|[up2`E3;\morphName{f}^2_0]
\arrow |a|/^{ (}->/[uint`up2;\morphName{i}]
\arrow |a|/>->/[uint`vhp2;\phi'^2]
\arrow |r|[uint`u3;\morphName{f}^2_0]
\arrow |r|[vhp2`c3;\morphName{f}^2_1]
...
If each $\morphSeqName{f}^i$ is an $\seqname{E}^1$-$\seqname{E}^{i+1}$
...
$
  \morphSeqName{f}^2 \morphCompose[()] \morphSeqName{f}^1 \defeq
  (
...
\begin{proof}
Since $\seqname{A}^2$ is semi-maximal and $\morphSeqName{f}^1$ is a
(near) morphism, $\morphSeqName{f}^1$
...
Similarly, $\morphSeqName{f}^2$ is a morphism.
...
\item
If each $\morphSeqName{f}^i$ is a (semi-strict, strict)
\nobreaks
  {
...
\item
If each $\morphSeqName{f}^i$ is a (semi-strict, strict) $\seqname{E}^1$
m-atlas morphism of $\seqname{A}^i$ to $\seqname{A}^{i+1}$ in the
...
$
  \hat{\morphName{f}} \defeq
  \phi^1 \morphCompose \phi^{2-1} \maps V'^2 \toiso V'^1
$.
...
\\
  &&
  \phi^1 \maps I \defeq U^1 \cap U^2 \morphMono V^1,
...
the coordinate space $\seqname{C}^i$,
$\morphSeqName{f} \defeq (\morphName{f}_0,\morphName{f}_1)$ be an
$\seqname{\seqname{E}}^1$-$\seqname{\seqname{E}}^2$ m-atlas (near)
morphism $(\morphName{f}_0,\morphName{f}_1)$ of $\seqname{A}^1$ to
...
m-atlas (near) morphism $(\morphName{f}_0,\morphName{f}_1)$ of
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces
...
\item If $\morphName{f} \maps A \to B$ is a morphism of
$
  \Functor^{\minimal{\M}}_2
    (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
$,
...
$
  \Functor^{\minimal{\M}}_2
    (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
$,
$A' \subseteq A$, $B' \subseteq B$ and $\morphName{f}[A'] \subseteq B'$
then since $\morphName{f} \maps A \to B$ is a morphism it is a
restriction of a transition function between sets in
$\pi_2[\seqname{A}^i]$ and its restrictions are also, hence morphisms,
and thus $\morphName{f} \restrictto_{A'} A' \to B'$ is a morphism.

\item
If $(U,V,\phi) \in \seqname{A}^i$ then
$\Id[{V}] = \phi \morphCompose \phi^{-1}$ is a transition function and hence a
morphism of
$
  \Functor^{\minimal{\M}}_2
    (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
...
$\morphName{i} \maps A' \hookrightarrow A$ is a restriction of an
...
$
    \Functor^{\minimal{\M}}_2
      (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
$
\item $\morphName{f}_\alpha \maps U_\alpha \to V_\alpha$ be a morphism of
$
      \Functor^{\minimal{\M}}_2
        (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
$
...
\item $\morphName{f} \maps U \to V$ be continuous and
$
  \uquant
    {{\alpha \prec \Alpha},{x \in U_\alpha}}
    {\morphName{f}(x) = \morphName{f}_\alpha(x)}
$
\end{enumerate}
Then $\morphName{f}$ is a morphism of $\seqname{C}^i$ and hence a
morphism of
$
      \Functor^{\minimal{\M}}_2
        (\seqname{A}^i, \seqname{E}^i, \seqname{C}^i)
...
Let $\morphSeqName{f} \defeq (\morphName{f}_0, \morphName{f}_1)$ be an
$\seqname{E}^1$-$\seqname{E}^2$ m-atlas (near) morphism from
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces
$\seqname{C}^1$, $\seqname{C}^2$.  If $\morphSeqName{f}$ is a morphism or
...
\begin{proof}
$\morphSeqName{f}$ is a morphism either by hypothesis or by
\pagecref{lem:M-ATLmorph}.
Let
$
  v^1 \in
...
$u^1 \defeq \phi^{1-1}(v^1)$, $u^2 \defeq \morphName{f}_0(u^1)$ and
$(U^2,V^2,\phi^2) \in \seqname{A}^2$ be a chart at $u^2$.
...
m-atlas morphism $(\morphName{f}_0,\morphName{f}_1)$ of
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces
$\seqname{C}^1$, $\seqname{C}^2$ is
...
$(\morphName{f}_0,\morphName{f}_1)$ of $\seqname{A}^1$ to $\seqname{A}^2$
in the coordinate spaces $\seqname{C}^1$, $\seqname{C}^2$.
\end{definition}
...
m^i \defeq
  \bigl (
    (\morphName{f}^i_0, \morphName{f}^i_1),
    o^i,
    o^{i+1}
  \bigr )
$
...
\item[Preservation of endpoints:]
$
\Functor^{\minimal{\M}}_1(\morphName{m}^i) =
\morphName{f}^i_0 \maps \Functor^{\minimal{\M}}_1 o^i \to
\Functor^{\minimal{\M}}_1 o^{i+1}
$
\item[Composition:]
\begin{align*}
  \Functor^{\minimal{\M}}_1(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
  & =
  \Functor^{\minimal{\M}}_1
    \bigl (
      (
...
\\*
  & =
  \morphName{f}^2_0 \morphCompose \morphName{f}^1_0 \maps
   \Functor^{\minimal{\M}}_1 o^1 \to
    \Functor^{\minimal{\M}}_1 o^3
\\*
  & =
  \bigl (
    \morphName{f}^2_0 \maps
    \Functor^{\minimal{\M}}_1 o^2 \to
    \Functor^{\minimal{\M}}_1 o^3
   \bigr )
  \morphCompose
  \bigl (
    \morphName{f}^1_0 \maps
    \Functor^{\minimal{\M}}_1 o^1 \to
...
\\*
  & =
  \Functor^{\minimal{\M}}_1(\morphName{m}^2)
  \morphCompose
  \Functor^{\minimal{\M}}_1(\morphName{m}^1)
\end{align*}
...
$
\Functor^{\minimal{\M}}_2(\morphName{m}^i) =
\morphName{f}^i_1 \maps \Functor^{\minimal{\M}}_2 o^i \to
\Functor^{\minimal{\M}}_2 o^{i+1}
$
\item $\Functor (g \morphCompose f) = \Functor (g) \morphCompose \Functor (f)$:
$
\Functor^{\minimal{\M}}_2(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
=
\\
\Functor^{\minimal{\M}}_2
  \bigl (
...
\\
\bigl (
  \morphName{f}^2_1 \maps
  \Functor^{\minimal{\M}}_2 o^2 \to
  \Functor^{\minimal{\M}}_2 o^3
  \bigr )
\morphCompose
\bigl (
  \morphName{f}^1_1 \maps
  \Functor^{\minimal{\M}}_2 o^1 \to
...
=
\\
\Functor^{\minimal{\M}}_2(\morphName{m}^2)
\morphCompose
\Functor^{\minimal{\M}}_2(\morphName{m}^1)
$
...
continuous functions $\morphName{f}_i \maps U_i \to U_{i+1}$ and
$\morphName{f}'_j \maps U_j \to U_{j+1}$ between the spaces:

\begin{align*}
D = \{
  &
    \morphName{f}_0 \maps U_0 = V_0 \to U_1,
    \dotsc,
    \morphName{f}_{m - 1} \maps U_{m - 1} \to U_m,
\\
  &
    \morphName{f}'_0 \maps  U_0 = V_0 \to V_1,
    \dotsc,
    \morphName{f}'_{m-1} \maps V_{m-1} \to V_n
\}
\end{align*}
...
diffeomorphism $\hat{\morphName{f}} \maps U_m \toiso V_n$ making the
...
$(U', V'', \phi' \morphCompose \phi) \in \seqname{A}$.
...
$\Ck$ diffeomorphism, $(U', V'', \phi' \morphCompose \phi)$ is $\Ck$
compatible with the charts of $\seqname{A}$.  Since $\seqname{A}$ is
maximal, $(U', V'', \phi' \morphCompose \phi)$ is a chart of $\seqname{A}$.
\end{proof}
\end{lemma}
...
$\morphSeqName{f}$ is an $E^1$-$E^2$ $\Ck$ near morphism of
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces $C^1$,
$C^2$, abbreviated as
$
\isAtl[\Ck,near]_\Ar
...
$\hat{\morphName{f}} \maps \hat{V}'^2 \toiso V'^2$ such that
...
space $C^i$ and
$\morphSeqName{f} \defeq (\morphName{f}_0, \morphName{f}_1)$ an $E^1$-$E^2$
$\Ck$ near morphism of $\seqname{A}^1$ to $\seqname{A}^2$ in the
coordinate spaces $C^1$, $C^2$. Then $\morphSeqName{f}$ is an $E^1$-$E^2$
...
the coordinate space $C^i$, $\morphName{f}_0 \maps E^1 \to E^2$ a
continuous function and $\morphName{f}_1 \maps C^1 \to C^2$ a $\Ck$
function. Then
...
$A, A', B, B'$ model meighborhoods of
$\Functor^{\minimal{\Ck}}_2 (\seqname{A}^i, E^i, C^i)$,
$A' \subseteq A$, $B' \subseteq B$ and $\morphName{f}[A'] \subseteq B'$ then since
$\morphName{f} \maps A \to B$ is a morphism it is a restriction of a transition
function between its restrictions to sets in $\pi_2[\seqname{A}^i]$ and
its restrictions are also, hence morphisms, and thus
$\morphName{f} \restrictto_{A'} A' \to B'$ is a morphism.

\item
If $(U,V,\phi) \in \seqname{A}^i$ then $\Id[{V}] = \phi \morphCompose \phi^{-1}A$
...
$
\end{enumerate}
Then $\morphName{f}$ is $\Ck$ and hence a morphism of
$
  \pi_2
    \biggl (
      \Functor^{\minimal{\Ck}}_2
...
coordinate space $C^i$, $\morphName{f}_0 \maps E^1 \to E^2$ a continuous
function, $\morphName{f}_1 \maps C^1 \to C^2$ a $\Ck$ function and
$\morphSeqName{f} \defeq (\morphName{f}_0, \morphName{f}_1)$ either a
...
\begin{proof}
$\morphSeqName{f}$ is a morphism either by \pagecref{cor:Ck-ATLmorph} or
...
\pagecref{fig:AtlM1}. Then $\morphName{f}_1 (v^1) \in \hat{V}'^2$.
\end{proof}
\end{lemma}
...
space $C^i$ and
$\morphSeqName{f} \defeq (\morphName{f}_0, \morphName{f}_1)$ an
...
The mapping associated with the $\seqname{E}^1$-$\seqname{E}^2$
$\Ck$-atlas morphism $(\morphName{f}_0,\morphName{f}_1)$ of
$\seqname{A}^1$ to $\seqname{A}^2$ in the coordinate spaces
$\seqname{C}^1$, $\seqname{C}^2$ is
...
\item[Preservation of endpoints:]
$
\Functor^{\minimal{\Ck}}_1(\morphName{m}^i) =
\morphName{f}^i_0 \maps \Functor^{\minimal{\Ck}}_1 o^i \to
\Functor^{\minimal{\Ck}}_1 o^{i+1}
$
\item[Composition:]
\begin{align*}
  \Functor^{\minimal{\Ck}}_1(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
  & =
  \Functor^{\minimal{\Ck}}_1
    \bigl (
      (
...
\\*
  & =
  \Functor^{\minimal{\Ck}}_1(\morphName{m}^2)
  \morphCompose
  \Functor^{\minimal{\Ck}}_1(\morphName{m}^1)
\end{align*}
...
\item[Preservation of endpoints:]\quad\\*
$
\Functor^{\minimal{\Ck}}_2(\morphName{m}^i) =
\morphName{f}^i_1 \maps \Functor^{\minimal{\Ck}}_2 o^i \to
\Functor^{\minimal{\Ck}}_2 o^{i+1}
$
\item $\Functor (g \morphCompose f) = \Functor (g) \morphCompose \Functor (f)$:
$
\Functor^{\minimal{\Ck}}_2(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
=
\\
\Functor^{\minimal{\Ck}}_2
  \bigl (
...
=
\\
\Functor^{\minimal{\Ck}}_2(\morphName{m}^2)
\morphCompose
\Functor^{\minimal{\Ck}}_2(\morphName{m}^1)
$
\item $\Functor (\Id[{A}]) = \Id_{\Functor (A)}$:
\begin{enumerate}
\item
...
\uquant
  {
   \morphName{f} \arin \catName{XY} \maps V \times Y \toiso V \times Y
  }
  {
    \equant
    {
...
$,
$G_{\seqname{XY},\morphName{f}} \defeq \range(\morphName{g})$
is the set of group elements associated with $\morphName{f}$
and
$
  G_\seqname{XY} \defeq
  \union%
    [{\morphName{f} \arin \catName{XY}}]
    {G_{\seqname{XY},\morphName{f}}}
$
...
\set
  {
   \morphName{f} \maps V \times Y \toiso V \times Y
  }%
  [
    {V \in \op{X}},
    {\pi_1 \morphCompose \morphName{f} = \pi_1},
    {
      \equant
        {
         \morphName{g} \in G^V,
...
$
  \morphSeqName{f} \defeq f_X \times f_Y \maps
  \Triv[G^1-\rho^1-]{X^1,Y^1} \to \Triv[G^2-\rho^2-]{X^2,Y^2}
$
a pair of continuous functions. Then $\morphSeqName{f}$ is a model
...
\begin{enumerate}
\item
$\morphSeqName{f}$ is a Cartesian product of continuous functions, hence
continuous.
...
$\morphSeqName{f}^{-1}(m^2) = U^1 \times Y^1$ is a model function.
...
$\morphSeqName{f}(m^1) \subseteq X^2 \times Y^2$.
\end{enumerate}
\end{proof}
\end{lemma}
...
whose links are continuous functions $\morphName{f}_i \maps U_i \to
U_{i+1}$ and $\morphName{f}'_j \maps U_j \to U_{j+1}$ between the spaces,
...
$\morphName{t}(x, y) = \bigl ( x, y \star \morphName{g}(x) \bigr )$.
Then $\pi_1 \morphCompose \morphName{t} = \pi_1$,
$\pi_1 \morphCompose \phi' = \pi_1 \morphCompose \phi =\pi$.
\end{proof}
\end{lemma}
...
$
  t^1_2 \defeq
  \phi^1 \morphCompose \phi^{2-1} \restrictto_{\phi^1[U^1 \cap U^2]}
$
...
$
is a homeomorphism.  It remains to show that
$\phi_2 \morphCompose \phi^{-1}_1 \restrictto_{\phi_1[U_1 \cap U_2]}$ is
...
$
\phi_2 \morphCompose \phi'^{-1}_\alpha \restrictto_{U_1 \cap U_2 \cap
U'_\alpha}
$
and
$\phi'_\alpha \morphCompose \phi^{-1}_1 \restrictto_{U^1 \cap U^2 \cap
U'_\alpha}
$
are generated by the group action and thus
$\phi^2 \morphCompose \phi^{-1}_1 = \phi^2 \morphCompose \phi'^{-1}_\alpha \morphCompose
\phi'_\alpha \morphCompose \phi^{-1}_1$
is generated by the group action.
\end{proof}
\end{lemma}
...
$(U', V'' \times Y, \phi' \morphCompose \phi) \in \seqname{A}$.
...
generated by the group action, $(U', V'' \times Y, \phi' \morphCompose \phi)$
is $G$-$\rho$-compatible with the charts of $\seqname{A}$. Since
$\seqname{A}$ is maximal, $(U', V'', \phi' \morphCompose \phi)$ is a chart of
$\seqname{A}$.
\end{proof}
\end{lemma}
...
\item For any charts
$(U^i, V^i, \phi^i \maps U^i \toiso V^i) \in \seqname{A}^i$. $i=1,2$,
with $I \defeq U^1 \cap \morphName{f}^{-1}_E[U^2] \neq \emptyset$,
...
\footnote
{
  ...
  This is permissible since $\hat{\morphName{f}}$ is a homeomorphism
}
...
\\
  &&
  \morphName{f}_E \maps I \defeq U^1 \cap \morphName{f}^{-1}_E[U^2] \to U^2,
  \phi^2 \maps U^2 \toiso V^2,
\\
  &&
  \phi^1 \maps I \morphMono V^1,
  \morphName{f}_X \times \morphName{f}_Y \maps V^1 \to X^2 \times Y^2
\\
& \bigr \}
\end{array}
\end{equation}
...
\begin{equation}
\label{eq:Bun-fup1in}
\morphName{f}_E[U'^1] \subseteq U'^2
\end{equation}
...
\begin{equation}
\morphName{f}_X \times \morphName{f}_Y[V'^1] \subseteq \hat{V}'^2
\label{eq:Bun-fvp1in}
\end{equation}
...
\begin{equation}
\hat{\morphName{f}} \morphCompose \phi^2 \morphCompose \morphName{f}_E =
\morphName{f}_X \times \morphName{f}_Y \morphCompose \phi^1
...
\node c21(1000,-1000)[C^2]
\node v2(2000,-1000)[V^2]
\arrow |a|[uint`u2;\morphName{f}_E]
\arrow |r|/>->/[uint`v1;\phi^1]
\arrow |r|[u2`v2;\phi^2]
\arrow |l|//[u2`v2;\iso]
\arrow |a|[v1`c21;\morphName{f}_X \times \morphName{f}_Y]
\efig \]
\caption{Uncompleted m-atlas near morphism}
\label{fig:Bun-AtlN1}
\end{figure}
...
\arrow |l|//[u2`v2;\iso]
\arrow |a|/_{ (}->/[vp1`v1;\morphName{i}]
\arrow |a|[vp1`vhp2;\morphName{f}_X \times \morphName{f}_Y]
\arrow |a|/ >.>>/[vp2`vhp2;\hat{\morphName{f}}]
\arrow |b|//[vp2`vhp2;\iso]
\arrow |a|/^{ (}->/[vp2`v2;\morphName{i}]
%\arrow |a|/^{ (}->/[v2`c2;\morphName{i}]
...
atlases then $\morphSeqName{f}$ is also a semi-maximal (maximal)
...
\item For any charts
$(U^i, V^i, \phi^i \maps U^i \toiso V^i) \in \seqname{A}^i$. $i=1,2$,
and any $u^1 \in I \defeq U^1 \cap \morphName{f}^{-1}_E[U^2]$,
...
\\
  ...
  \morphName{f}_X \times \morphName{f}_Y \maps V'^1 \to \hat{V}'^2,
\\
  &&
...
thus $(\morphName{f}_E, \morphName{f}_X \times \morphName{f}_Y)$ is an
...
to $\Triv[G^2-\rho^2-]{X^2,Y^2}$ then $\morphSeqName{f}$ commutes with
$\rho^i$.

\item
If $(\morphName{f}_E, \morphName{f}_X \times \morphName{f}_Y)$ is a
$\Triv{E^1}$-$\Triv{E^2}$ m-atlas morphism from $\seqname{A}^1$ to
$\seqname{A}^2$ in the coordinate spaces $\Triv[G^1-\rho^1-]{X^1,Y^1}$,
$\Triv[G^2-\rho^2-]{X^2,Y^2}$ then $\morphSeqName{f}$ commutes with
$\pi^i$.

A diagram is locally nearly commutative in $X^2,Y^2,\pi^2,\rho^2$ iff it
is m-locally nearly commutative in
$\pi_2 \biggl ( \Triv[G^2-\rho^2-]{X^2,Y^2} \biggr )$
thus $(\morphName{f}_E, \morphName{f}_X \times \morphName{f}_Y)$ is a
m-atlas morphism in the coordinate space $\Triv[G^2-\rho^2-]{X^2,Y^2}$,
so $\morphSeqName{f}$ is a $\seqname{B^1}$-$\seqname{B^2}$ bundle-atlas
...
If each $\seqname{A}^i$ is semi-maximal or if each $\morphSeqName{f}^i$
is a morphism then $\morphSeqName{f}^2 \morphCompose[()] \morphSeqName{f}^1$ is
...
Then $\morphSeqName{f}^{i+1} \morphCompose[()] \morphName{f}^1$ is $\Ck$-equivalent
to $\morphSeqName{g}^2 \morphCompose[()] \morphName{g}^1$.
...
$\pi_1 \morphCompose \phi = \pi(\seqname{A}^\alpha) \restrictto_U$
...
$\morphSeqName{f}$ from $\seqname{A^1}$ to $\seqname{A^2}$ is
...
$\seqname{B^1}$-$\seqname{B^2}$ bundle-atlas (near) morphism
$\morphSeqName{f}$ from $\seqname{A^1}$ to $\seqname{A^2}$.
\end{definition}
...
$\morphName{f}[A'] \subseteq B'$ then since $\morphName{f} \maps A \to B$
...
\item
If $(U,V,\phi) \in \seqname{A}$ then $\Id[{V}] = \phi \morphCompose \phi^{-1}A$
is a transition function and hence a morphism of
...
$
\end{enumerate}
Then $\morphName{f}$ is generated by the group action and hence a
...
bundle-atlas (near) morphism $\morphSeqName{f}$ from $\seqname{A^1}$ to
$\seqname{A^2}$ is
...
(near) morphism $\morphSeqName{f}$ from $\seqname{A^1}$ to
$\seqname{A^2}$.
\end{definition}
...
\\*
  & =
  \morphName{f}^i_E \maps \Functor^\Bun_1 o^i \to o^{i+1}
\end{align*}
by \pagecref{def:ismodT}.

\item[Composition:]
\begin{align*}
  \Functor^\Bun_1(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
  & =
  \Functor^\Bun_1
    \bigl (
      \morphSeqName{f}^2 \morphCompose \morphSeqName{f}^1
...
\\*
  & =
  \morphName{f}^2_E \morphCompose \morphName{f}^1_E \maps
    \Functor^\Bun_1 o^1 \to
    \Functor^\Bun_1 o^3
\\*
  & =
  \bigl (
    \morphName{f}^2_E \maps
    \Functor^\Bun_1 o^2 \to
    \Functor^\Bun_1 o^3
    \bigr )
  \morphCompose
...
\\*
  & =
  \Functor^\Bun_1 \morphName{m}^2
  \morphCompose
  \Functor^\Bun_1 \morphName{m}^1
\end{align*}
...
$
\Functor^\Bun_2(\morphName{m}^i) =
\morphName{f}^i_1 \maps \Functor^\Bun_2 o^i \to
\Functor^\Bun_2 o^{i+1}
$
\item[Composition:]
\begin{align*}
  \Functor^\Bun_2(\morphName{m}^2 \morphCompose[A] \morphName{m}^1)
  & =
\Functor^\Bun_2
  \bigl (
    (
...
\\*
  & =
\Functor^\Bun_2(\morphName{m}^2)
\morphCompose
\Functor^\Bun_2(\morphName{m}^1)
\end{align*}
...
\label{def:Bun}
Let $E$, $X$ and $Y$ be topological spaces, $\pi \maps E \morphEpi X$ surjective,
