...
% \into NOT renamed 2026-09-01, unlike the rest of this migration pass:
% found via direct visual comparison that \mon (Xy-pic, what \into
% wraps with \negthickspace) is NOT the same glyph as semantic-markup's
% \morphMono (\hookrightarrow) -- \mon is Xy-pic's own thin small-tail
% arrow, visually distinct from the standard hooked arrow. Renaming
% the call sites would silently change what's printed. Left as-is,
% pending a decision on whether \lsm_style_default:'s \morphMono
% should instead render as \mon to match. See
